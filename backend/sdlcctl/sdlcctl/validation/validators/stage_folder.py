...
Validates SDLC 6.0.6 stage folder structure and naming.
...
class StageFolderValidator(BaseValidator):
    ...
    VALIDATOR_DESCRIPTION = "Validates SDLC 6.0.6 stage folder structure"

    # Stage pattern: 2 digits + hyphen + name
    STAGE_PATTERN = re.compile(r"^(\d{2})-(.+)$")

    # Valid stage IDs (00-10 in SDLC 6.0.6)
    ...
    def _check_unknown_stages(self, stage_folders: List[Path]) -> List[ViolationReport]:
        ...
        Valid stage IDs: 00-10 (SDLC 6.0.6)
