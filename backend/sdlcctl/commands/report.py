...
def _generate_markdown_report(result: ValidationResult) -> str:
    ...
    for stage_id in sorted(STAGE_NAMES.keys()):
        ...
        stages_rows.append(
            f"| {stage_id} | {stage_name} | {'Required' if required else 'Optional'} | {status} | {files} | {readme} |"
        )
    ...
    p0_section = "\n### P0 Artifacts\n\n"
    p0_section += f"- **Required for Tier**: {'Yes' if result.tier_requirements.p0_required else 'No'}\n"
...
| Metric | Value |
|--------|-------|
| **Status** | {status_emoji} {'COMPLIANT' if result.is_compliant else 'NON-COMPLIANT'} |
...
"""
    ...
    if recommendations:
        return f"""{_generate_markdown_report.__doc__}
