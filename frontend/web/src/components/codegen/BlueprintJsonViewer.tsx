/**
 * File: frontend/web/src/components/codegen/BlueprintJsonViewer.tsx
 * Version: 1.0.0
 ...
 *
 * Description:
 * AppBlueprint JSON viewer with collapsible sections and copy functionality.
 * Provides both full JSON view and structured section view.
 */
...
// AppBlueprint types based on backend IR schema
interface AppBlueprint {
  app_name: string
  app_name_display: string
  domain: string
  version: string
  sdlc_tier: string
...
export function BlueprintJsonViewer({
  ...
  const totalEntities = blueprint.modules?.reduce(
    (sum, m) => sum + (m.entities?.length || 0),
    0
  ) || 0
  const totalEndpoints = blueprint.modules?.reduce(
      ...
      <CardContent>
        <Tabs defaultValue="structured" className="w-full">
          ...
          <TabsContent value="structured" className="space-y-2">
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-start justify-between">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleCopySection(`module-${idx}`, module)}
                        >
            ...
            {/* Config Section */}
            <CollapsibleSection title="Cấu hình hệ thống">
              <div className="space-y-3 mt-2">
            ...
            <CollapsibleSection title="Metadata">
              <div className="mt-2 p-3 rounded-lg bg-muted/50 text-sm space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{new Date(blueprint.metadata?.generated_at || Date.now()).toLocaleString('vi-VN')}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{blueprint.metadata?.estimated_lines?.toLocaleString() || '~5,000'}</span>
                </div>
              </div>
            </CollapsibleSection>
          </TabsContent>
