...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        // Simplified manual chunks strategy to avoid circular dependency issues
        // The "Cannot access 'Yn' before initialization" error occurs when
        // modules with circular dependencies are split into separate chunks
        manualChunks: {
          ...
          // Bundle TanStack Query separately (no circular deps with React)
          'query-vendor': [
            '@tanstack/react-query',
            '@tanstack/react-query-devtools',
          ],
          // Date utilities (standalone, no circular deps)
          'date-vendor': [
            'date-fns',
          ],
        },
