    ...
    analyzeExistingStructure(workspaceRoot: string, targetTier: SDLCTier): GapAnalysisResult {
                ...
                for (const pattern of patterns) {
                    if (fs.existsSync(path.join(workspaceRoot, pattern))) {
                        ...
                        recommendations.push(
                            `Consider renaming '${pattern}' to '${expectedFolder}' for SDLC 6.0.6 compliance`
                        );
                        found = true;
                        break;
                    }
        ...
        for (const legacy of legacyFolders) {
            ...
            if (fs.existsSync(legacyPath)) {
                recommendations.push(
                    `Found legacy SDLC 4.9.x folder 'docs/${legacy}'. Run 'sdlcctl migrate' to upgrade to 6.0.6`
                );
            }
        }
